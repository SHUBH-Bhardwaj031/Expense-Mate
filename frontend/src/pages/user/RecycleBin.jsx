...
const res = await axiosInstance.get("/expenses/deleted", { params });
      const data = res?.data;
  ...
  const restoreExpenses = async (ids) => {
   ...
   await Promise.all(
  ids.map(id =>
    axiosInstance.put(`/expenses/restore/${id}`)
  )
);
  ...
  const deleteForever = async (ids) => {
    ...
    await Promise.all(
  ids.map(id =>
   axiosInstance.delete(`/expenses/permanent/${id}`)
  )
);
      ...
      <div className="mb-4 grid grid-cols-1 md:grid-cols-12 gap-3">
        <div className="md:col-span-4 relative">
          ...
          <input
            ...
            className="w-full rounded-lg border bg-white border-gray-300 pl-9 pr-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-400"
            value={searchTerm}
        ...
        <div className="md:col-span-3 relative">
          ...
          <select
            className="w-full appearance-none rounded-lg  bg-white border border-gray-300 pl-9 pr-9 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white"
            value={category}
        ...
        <div className="md:col-span-2 relative">
          ...
          <input
            type="date"
            className="w-full rounded-lg bg-white border border-gray-300 pl-9 pr-3 py-2.5 focus:outline-none focus:ring-2 focus:ring-blue-400"
            value={startDate}
